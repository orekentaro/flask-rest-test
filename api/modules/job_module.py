    ...
    def get_job_seeker(cls, **conditions):
        try:
            with cls.session_scope() as db_session:
                job_seekers = (
                    db_session.query(JobSeeker)
                    .filter_by(**conditions, delete_flag=cls.const.DELETE_FLAG_OFF)
                    .all()
                )
                ...
                for job_seeker in job_seekers:
                    ...
                    job = (
                        db_session.query(JobMaster, JobAds)
                        .join(JobAds, JobMaster.job_master_id == JobAds.job_master_id)
                        .filter(
                    ...
                    progress = (
                        db_session.query(ProgressInfo, ProgressResult, ProgressMaster)
                        .join(
                            ProgressMaster,
                    ...
                    user = cls.get_user(**{"user_id": pi.user_id}) or {}
                    ...
                    res_dict = {
                        ...
                        "progress": [],
                        "memo": [],
                        "create_time": cls.date_to_string(job_seeker.create_time),
                    }
                    ...
                    for prg in progress_all:
                        ...
                        user = cls.get_user(**{"user_id": pi_all.user_id}) or {}
                    ...
                    memos = (
                        db_session.query(Memo)
                        .filter_by(job_id=job_id, delete_flag=cls.const.DELETE_FLAG_OFF)
                        .all()
                    )

                    for memo in memos:
                        create_time = cls.date_to_string(memo.create_time)
                        user = cls.get_user(**{"user_id": memo.changer}) or {}
                        memo_dict = {
